...
/**
 * Servlet implementation class ReimbursementsDepHead.
 */
public class ReimbursementsDepHead extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ((rr != null) && !rr.isEmpty()) {
			...
			for (ReimbursementRequest r : RegisterReimbursements.register(request)) {
				out.println("<tr id ='" + r.getREIMBURSE_ID() + "'>" + "<td>"
						...
						+ "</td>" + "<td>" + r.getTIMEOFF_START() + "</td>" + "<td>"
						+ r.getTIMEOFF_END() + "</td>" + "<td>" + r.getREIMBURSE_LOCATION()
						+ "</td>" + "<td>$" + r.coveredCost() + "</td>" + "<td>"
						+ "<form action = 'ApproveStatusDepHead'>"
						+ "<input type = 'hidden' name='rid' value =" + r.getREIMBURSE_ID()
						+ ">"
						+ "<button type = 'submit' class='btn btn-block center-block btn-primary btn-sm'>"
						+ "Approve" + "</button>" + "</form>" + "<form action = 'DenyStatus'>"
						+ "<input type = 'hidden' name='rid' value =" + r.getREIMBURSE_ID()
						+ ">"
						+ "<button type = 'submit' class='btn btn-block center-block btn-primary btn-sm'>"
						+ "Deny" + "</button>" + "</form>" + "</td>" + "</tr>");
			}
		...
		out.println("<hr>" + "<a href='dephead.html'>BACK</a>");
	}
